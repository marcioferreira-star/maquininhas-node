...
**Estado (10/07):** Neon criado · schema aplicado · staging carregado (100%) · promoção → tabelas finais OK (552 máq / 1960 eventos / 1354 mov) · **sync automático Planilha→Neon PRONTO** (`src/sync-neon.js` + endpoint `/api/sync-neon` + Vercel Cron a cada 15 min; mão única — planilha é a fonte viva, Neon é espelho). Falta ativar em prod: `DATABASE_URL` + `CRON_SECRET` nas envs da Vercel. Cutover (app rodando NO Neon) = fases 3-6, por fazer.
